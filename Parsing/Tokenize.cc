...
namespace Kodu {
	...
	bool TokenParser::tokenize(const std::string& kString, std::vector<TokenBase*>& tokens) {
		std::size_t pos = 0;
		...
		while (pos < kSize) {
			...
			if (isdigit(currChar)) {
				...
				// ASSERTION: the first character of the number sequence is not a zero (unless followed by a zero)
				if (kString[pos] == '0') {
					PARSER_ASSERT((kString[pos + 1] == '.'),
						errorMessage << "(Col " << (pos + 2)
						<< ") A number can only begin with zero if it is below 1.");
				}
