...
namespace Kodu {
	...
	KoduActionMotion* KodeCreator::createMoveKode(std::vector<TokenBase*>& mods) {
		// ASSERTION: There are
	...
	KoduActionPlay* KodeCreator::createPlayKode(std::vector<TokenBase*>& mods) {
	...
	KoduActionSay* KodeCreator::createSayKode(std::vector<TokenBase*>& mods) {
		...
		while (!mods.empty()) {
			...
			// check if the token is a keyword
				...
				else if (ColorTypes::isValidColor(keyword) != -1) {
		...
		if (color.empty())
			 color = ColorTypes::getDefaultColor();
	...
	KoduActionScore* KodeCreator::createScoreKode(const std::string& scoreType,
		std::vector<TokenBase*>& mods)
	{
		...
		if (scoreDesignator.empty())
			scoreDesignator = ColorTypes::getDefaultColor();
		// create the action
		return (new KoduActionScore(scoreOp, tempNumGen, scoreDesignator, onceEnabled));
	}

	KoduActionMotion* KodeCreator::createTurnKode(std::vector<TokenBase*>& mods) {
		// ASSERTION: There are
		PARSER_ASSERT((0 <= mods.size() && mods.size() <= 5),
