...
namespace Kodu {
	...
	KoduConditionSee* KodeCreator::createSeeKode(std::vector<TokenBase*>& mods) {
		// mandatory modifiers
		...
		while (!mods.empty()) {
			...
			if (mods[0]->isKeywordToken()) {
				...
				else if (ColorTypes::isValidColor(keyword)) {
	...
	KoduConditionScored* KodeCreator::createScoredKode(std::vector<TokenBase*>& mods) {
		// optional modifiers
	...
	KoduConditionTimer* KodeCreator::createTimerKode(std::vector<TokenBase*>& mods) {
